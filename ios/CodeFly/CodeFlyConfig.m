...
@implementation CodeFlyConfig {
    NSMutableDictionary *_configDictionary;
}

static CodeFlyConfig *_currentConfig;
...
+ (void)initialize
{
    if (self == [CodeFlyConfig class]) {
        _currentConfig = [[CodeFlyConfig alloc] init];
    }
}
...
- (instancetype)init
{
    ...
    NSString *deploymentKey = [infoDictionary objectForKey:@"CodeFlyDeploymentKey"];
    NSString *serverURL = [infoDictionary objectForKey:@"CodeFlyServerURL"];
    NSString *publicKey = [infoDictionary objectForKey:@"CodeFlyPublicKey"];
    ...
    if (!serverURL) {
        serverURL = @"https://CodeFly.appcenter.ms/";
    }
